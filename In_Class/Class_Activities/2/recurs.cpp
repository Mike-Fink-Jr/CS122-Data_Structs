...
{
    if (value >= 10)		// more than one digit 
	return firstDigit( value / 10 );
    else			// exactly one digit
	return value;
}
...
int gcf( int a, int b )
{
    ...
    rem = a % b;
    if (rem > 0)		// no common factor yet found
	return gcf( b, rem );
    else			// b divides both a and b
	return b;
}
...
int bsearch( const int list[], int value, int left, int right )
{
	...
	return -1;
    else if (value == list[mid])	// found!
	return mid;
    else if (value < list[mid])		// value would be in lower half
	return bsearch( list, value, left, mid-1 );
    else				// value would be in upper half
	return bsearch( list, value, mid+1, right );
}
