...
void SingleList::insert(int newValue)
{
	...
	if (newNode->value < head->value)				// condition needs a change
	{
		newNode->next = head;
		head = newNode;
	...
	{
		scan = head;
		while (scan->next != NULL && scan->next->value < newNode->value)				// condition needs a change
			scan = scan->next;
		if (scan->next == NULL)
		{
...
void SingleList::remove(int value)
{
	...
	while (scan->next != NULL && scan->next->value < value)
		scan = scan->next;

	if (scan->next != NULL && scan->next->value == value)
	{
								removal = scan->next;
...
void DoubleList::insert(int newValue)
{
	...
	else
	{
		...
		scan->next->prev = newNode;// insert after scanned node
		scan->next = newNode;
		newNode->prev = scan;
